...
const StatisticsPage = ({ results, onBack, quizzes }) => {
    ...
    const handleExportCSV = () => {
        const headers = ["Reytinq", "Ad", "Soyad", "Test Adı", "Nəticə (Düzgün)", "Nəticə (Ümumi)", "Faiz", "Tarix"];
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {sortedResults.map((result, index) => (
                                <tr key={result.id}><td className="px-3 py-3 whitespace-nowrap text-sm text-gray-500">{index + 1}</td><td className="px-3 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{result.userName} {result.userSurname}</td><td className="px-3 py-3 whitespace-nowrap text-sm text-gray-900">{result.quizTitle}</td><td className="px-3 py-3 whitespace-nowrap text-sm text-gray-500">{result.score} / {result.total}</td><td className="px-3 py-3 whitespace-nowrap text-sm text-gray-500">{result.percentage}%</td><td className="px-3 py-3 whitespace-nowrap text-sm text-gray-500">{new Date(result.date).toLocaleDateString('az-AZ')}</td></tr>
                            ))}
                        </tbody>
                    </table>
                </div>
